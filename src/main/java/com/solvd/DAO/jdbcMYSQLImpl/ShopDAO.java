...
public class ShopDAO extends AbstractDAO implements IShopDAO {
    ...
    private final static String UPDATE = "UPDATE Shops SET name=?,web_page=?, phone_number=?, idOwner=?, idClients=? WHERE idShops=?";
    private final static String SELECT = "SELECT * FROM Shops WHERE idShops=?";
    private final static String DELETE = "DELETE FROM Shops WHERE idShops=?";
    ...
    public Shop getEntityById(long id) {
        ...
        try {
            ...
            shop.setName(rs.getNString("name"));
            shop.setPhoneNumber(Integer.parseInt(rs.getNString("phone_number")));
            shop.setWebPage(rs.getNString("web_page"));
